import streamlit as st
from openai import AzureOpenAI
from prompts import SYSTEM_PROMPT, generate_user_prompt
from configs.scenario_config import GENDER_OPTIONS, AGE_RANGE, ACCIDENT_TYPE_OPTIONS, CAR_MODEL_OPTIONS, CAR_COLOR_OPTIONS, NUM_PASSENGERS_RANGE, LOCATION_OPTIONS, SEVERITY_OPTIONS
from configs.model_config import MODEL_CONFIG
from dotenv import load_dotenv
import os
import random

load_dotenv()

# Initialize Azure OpenAI client
client = AzureOpenAI(
    api_key=os.getenv('OPENAI_API_KEY'),
    api_version=MODEL_CONFIG['api_version'],
    azure_endpoint=MODEL_CONFIG['endpoint']
)

# No longer need random names as they will be generated by the LLM

st.title("Accident Scenario Generator")
st.write("This application generates an accident scenario based on your configuration settings on the left. Once configured, press the 'Generate Accident Scenario' button to see the generated scenario.")

st.sidebar.header("Configure Scenario")
gender = st.sidebar.selectbox("Gender:", GENDER_OPTIONS)
age = st.sidebar.number_input("Age:", **AGE_RANGE)
car_model = st.sidebar.selectbox("Car Model:", CAR_MODEL_OPTIONS)
car_color = st.sidebar.selectbox("Car Color:", CAR_COLOR_OPTIONS)
num_passengers = st.sidebar.number_input("Number of Passengers:", **NUM_PASSENGERS_RANGE)
location = st.sidebar.selectbox("Location:", LOCATION_OPTIONS)
severity = st.sidebar.selectbox("Severity of Accident:", SEVERITY_OPTIONS)

# Name will be generated by the LLM

if st.button("Generate Accident Scenario", key="generate_button", help="Click to generate a scenario", use_container_width=True):
    try:
        with st.spinner('Generating scenario...'):
            prompt = generate_user_prompt(gender, age, car_model, car_color, num_passengers, location, severity)
            response = client.chat.completions.create(
                model=MODEL_CONFIG['engine'],
                messages=[{"role": "system", "content": SYSTEM_PROMPT}, {"role": "user", "content": prompt}],
                max_tokens=MODEL_CONFIG['max_tokens'],
                temperature=MODEL_CONFIG['temperature'],
                top_p=MODEL_CONFIG['top_p']
            )
            
            # Extract content from the response with error handling
            if hasattr(response, 'choices') and len(response.choices) > 0 and hasattr(response.choices[0].message, 'content'):
                content = response.choices[0].message.content
                
                # Display the generated content
                st.text_area("Result:", content.strip(), height=500, max_chars=None, key=None, help=None, 
                            on_change=None, args=None, kwargs=None, placeholder=None, disabled=False, 
                            label_visibility="visible")
                
                # Button to save output to a markdown file
                st.download_button(
                    label="Save to Markdown",
                    data=content.strip(),
                    file_name="accident_scenario.md",
                    mime="text/markdown"
                )
            else:
                st.error("Failed to generate content. The API response was incomplete or in an unexpected format.")
    except Exception as e:
        st.error(f"An error occurred: {str(e)}")
        st.info("Please try again or adjust your parameters.")
